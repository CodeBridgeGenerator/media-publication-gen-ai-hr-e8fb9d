...
import jobClassifiedFakerFactory from "./jobClassifiedFakerFactory";
...
const JobClassifiedFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [employeeNameIds, setemployeeNameIds] = useState();
const [employeeInterestIds, setemployeeInterestIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("employeeDetails")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setemployeeNameIds(data.data);
                        });
client.service("employeeDetails")
                        .find({ query: { $select: ["_id","skills"] } })
                        .then((data) => {
                        setemployeeInterestIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = jobClassifiedFakerFactory(props.user,fakerCount,employeeNameIds,employeeInterestIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobClassified").create(elem));
...
export default connect(mapState, mapDispatch)(JobClassifiedFakerDialogComponent);
