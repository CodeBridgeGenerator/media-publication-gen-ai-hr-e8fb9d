...
import employeeDetailsFakerFactory from "./employeeDetailsFakerFactory";
...
const EmployeeDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = employeeDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("employeeDetails").create(elem));
...
export default connect(mapState, mapDispatch)(EmployeeDetailsFakerDialogComponent);
