...
const HumanResourcesCreateDialogComponent = (props) => {
...
employee: _entity?.employee,
        ...
        const result = await client.service("humanResources").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info humanResources updated successfully" });
    ...
    return (
        <Dialog header="Edit HumanResources" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="humanResources-edit-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            ...
            <InputText id="employee" className="w-full mb-3 p-inputtext-sm" value={_entity?.employee} onChange={(e) => setValByKey("employee", e.target.value)}  />
        </span>
        </div>
...
export default connect(mapState, mapDispatch)(HumanResourcesCreateDialogComponent);
