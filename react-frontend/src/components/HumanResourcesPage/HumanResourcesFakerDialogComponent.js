...
import humanResourcesFakerFactory from "./humanResourcesFakerFactory";
...
const HumanResourcesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = humanResourcesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("humanResources").create(elem));
...
export default connect(mapState, mapDispatch)(HumanResourcesFakerDialogComponent);
